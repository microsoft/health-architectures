...
__Important:__ Be sure to make one of the environments you imported active (`api-for-fhir` or `fhir-proxy`) by selecting from the dropdown menu above the `Send` button. In the image below, `fhir-proxy` is the active environment.
