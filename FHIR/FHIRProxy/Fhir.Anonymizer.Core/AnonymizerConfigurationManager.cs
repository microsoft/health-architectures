...
namespace Fhir.Anonymizer.Core
{
    public sealed class AnonymizerConfigurationManager
    {
        ...
        public AnonymizerConfigurationManager(AnonymizerConfiguration configuration)
        {
            ...
            if (_configuration.PathRules != null)
            {
                _resourcePathRules = _configuration.PathRules.Where(entry => IsResourcePathRule(entry.Key))
                    ...
                    .ToDictionary(group => group.Key, group => group.Select(item => new AnonymizerRule(item.Key, item.Value)));
            }
            else
            {
        ...
        public static AnonymizerConfigurationManager CreateFromConfigurationFile(string configFilePath)
        {
            try
            {
                return CreateFromConfigurationString(File.ReadAllText(configFilePath));
            }
        ...
        public static AnonymizerConfigurationManager CreateFromStream(Stream s)
        {
            try
            {
                StreamReader reader = new StreamReader(s);
                string text = reader.ReadToEnd();
                return CreateFromConfigurationString(text);
            }

            catch (IOException innerException)
            {
                throw new IOException($"Failed to read configuration file from stream", innerException);
            }
            catch (JsonException innerException)
            {
                throw new JsonException($"Failed to parse configuration file from stream", innerException);
            }

        }
        public static AnonymizerConfigurationManager CreateFromConfigurationString(string content)
        {
            try
            {
            ...
            {
                throw new JsonException($"Failed to parse configuration file from string:{content}", innerException);
            }
        }
